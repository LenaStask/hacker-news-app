...
function CommentListItem ({ comment, level }: { comment: IComment, level: number }): JSX.Element {
  ...
  return (
    <div>
      <Link className='commentLink' onClick={() => { openComments(comment) }}>
        <div dangerouslySetInnerHTML={{ __html: comment.text }}></div>
      </Link>
      { hasKids
        ? <CommentTree parentId={comment.id} level={level + 1}/>
        : ''
      }
    </div>
  )
